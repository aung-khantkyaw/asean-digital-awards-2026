...
const ADMIN_HEADER_THEME = {
  ...
  languageButtonClass:
    "border border-white/70 bg-white/10 text-white hover:bg-white/20 focus-visible:ring-2 focus-visible:ring-white/70 focus-visible:ring-offset-0",
  languageDividerClass: "border-white/30",
};
    ...
    <header className={headerClasses} data-user-role={roleAttribute}>
      <div className="mx-auto max-w-screen-xl flex h-14 items-center gap-3 px-4 sm:px-6">
        <div className="flex items-center gap-2 min-w-0 flex-shrink-0">
          <div className="h-8 w-8 rounded-md flex items-center justify-center text-white text-[10px] font-semibold shadow-sm select-none">
        ...
        <nav
          aria-label="Primary"
          className="flex flex-1 items-center justify-center gap-5 text-sm font-medium"
        >
          {primaryLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="text-current transition-colors whitespace-nowrap"
            >
              {link.label}
            </Link>
          ))}
